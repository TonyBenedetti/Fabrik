/**
 * The form has just been loaded or the user has just chosen a division of a
 * year: -- division possibilities are: month, quarter, third, half & season --
 * change the label on the "_picklist" element -- update "_picklist" element
 * with the value that may have been previously saved
 * 
 * @param {Object}
 *            thisElement - Fabrik element that called us via onLoad or onClick.
 */
...
/**
 * The form has just been loaded or the user has just chosen a value for this
 * division of a year: -- division possibilities are: month, quarter, third,
 * half & season -- stow away that value in the corresponding "_saver_" element
 * 
 * @param {Object}
 *            thisElement - Fabrik element that called us via onLoad or onClick.
 */
 ...
 * 
 * @param {Object}
 *            thisElement - The Fabrik element calling us via onLoad or onClick.
 * @param {string}
 *            tabName - "start", "duration", or "end"
 * @todo Get Fourmilab calendar working
 */
/*
 * function commonDateCalendarType(thisElement, tabName) { return; // "beat it" -
 * until rest of function is ready to be tested var thisForm = thisElement.form;
 * var table = 'gn_event___' + tabName; var baseName = table + '_date'; var
 * lineName = baseName + '_division'; var calendars = baseName +
 * '_calendar_type'; var calendar = thisForm.elements.get(calendars).getValue();
 * switch(calendar) { case 'iso-edtf' : // change division & subdiv headers
 * break; case 'iso-yd' : // hide the division subgroup // set the number of
 * days for dropdown to ~365 break; case 'iso-ywd' : // hide the division
 * subgroup // set the number of weeks for dropdown to ~52 // set the number of
 * days for dropdown to 7 // set division header to "--- Week" break; case
 * 'julian' : break; } }
 */
 ...
 */
/*
 * function commonDateLineShowHide(thisElement, tabName, line, action) { var
 * thisForm = thisElement.form; var table, tableName, baseName, lineName, elems
 * tableName = thisForm.options.primaryKey.match(/(\w+)___\w+/)[1]; table =
 * tableName + tabName; baseName = table + '_date'; lineName = baseName + '_' +
 * line; elems[] = baseName + '_header'; elems[] = table + '_spacer_4a'; elems[] =
 * table + '_spacer_4b'; elems[] = table + '_spacer_5b'; elems[] = lineName;
 * elems[] = lineName + '_accuracy'; elems[] = lineName + '_confidence'; for (i =
 * 0, i < elems.length, i++) { thisForm.elements.get(elems[i].id).hide(); } }
 */

/*
 * function listAllProperties(o) { var objectToInspect; var result = [];
 * 
 * for(objectToInspect = o; objectToInspect !== null; objectToInspect =
 * Object.getPrototypeOf(objectToInspect)) { result =
