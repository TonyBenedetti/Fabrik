 ...
 * @subpackage  Fabrik.element.spacer
...
class PlgFabrik_ElementSpacer extends PlgFabrik_Element
{
	/**
	 * Db table field type
	 *
	...
	public function getLabel($repeatCounter = 0, $tmpl = '')
	{
		...
		if (!$params->get('spacer_showlabel', true))
		{
	...
	{
		if (!$this->getParams()->get('spacer_showlabel', true))
		{
	...
	public function render($data, $repeatCounter = 0)
	{
		...
		$spacerData = new stdClass;
		$spacerData->id = $this->getHTMLId($repeatCounter);
		$spacerData->value = $params->get('spacer_showlabel', true) ? $this->getValue($data, $repeatCounter) : '';

		return $layout->render($spacerData);
	}

	/**
	 * Helper method to get the default value used in getValue()
	 * Unlike other elements where readonly affects what is displayed,
	 * the spacer element is always read only, 
